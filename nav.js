...
st.textContent='.nav{'+(document.querySelector('.langbar')?'':'position:sticky;top:0;')+'background:rgba(7,11,9,.95);display:flex;justify-content:center;gap:4px;padding:8px;flex-wrap:wrap;z-index:9;border-bottom:1px solid rgba(245,196,81,.2)}.nav a{color:#f5c451;padding:6px 12px;border-radius:16px;font-size:.8rem;border:1px solid transparent;display:inline-flex;align-items:center;gap:6px}.nav a.on{border-color:rgba(245,196,81,.4);background:#101a15;box-shadow:0 0 12px rgba(245,196,81,.15)}.nic{width:18px;height:18px;stroke:url(#ng);fill:none;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 1px 2px rgba(0,0,0,.7)) drop-shadow(0 0 4px rgba(245,196,81,.45))}.tgfloat{position:fixed;bottom:18px;left:18px;z-index:99;width:58px;height:58px;border-radius:50%;background:linear-gradient(145deg,#5ecdf5,#2AABEE 60%,#1c8fc7);display:flex;align-items:center;justify-content:center;box-shadow:0 8px 25px rgba(42,171,238,.5);animation:fl 2.2s infinite}.tgfloat svg{width:28px;height:28px;fill:#fff}@keyframes fl{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}';
document.head.appendChild(st);
document.body.insertAdjacentHTML('beforeend','<a class="tgfloat" href="https://t.me/TurkaslaniFx_bot" target="_blank" aria-label="چت آنلاین با ربات"><svg viewBox="0 0 24 24"><path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71l-4.14-3.05-1.99 1.93c-.23.23-.42.42-.83.41z"/></svg></a>');
if(!el)return;
